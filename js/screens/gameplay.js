...
class GameplayScreen {
  ...
  async render(container) {
    ...
    // Check if this is playlist test mode (legacy)
    const testState = State.get('playlistTest');
    if (testState && Array.isArray(testState.songs)) {
  ...
  async playSongWithVinyl(container) {
    ...
    try {
      ...
      // Additional delay to ensure video is buffered
      ...
      // Show vinyl overlay AFTER video starts
  ...
  renderPlaylistTest() {
    // ... (keep existing playlist test code)
    // This is for testing songs from the database manager
    // Not part of the main gameplay flow
  }
}
