...
class DatabaseManagerScreen {
      ...
      <div class="group relative flex flex-col gap-3 rounded-2xl bg-surface-dark p-4 shadow-sm border border-white/5 transition-all hover:border-primary/50">
        ...
        <div class="flex justify-between items-start">
          ...
          <div class="flex flex-col gap-1 min-w-0 pr-4">
            ...
            <p class="text-sm text-white/70 font-medium truncate">${Utils.escapeHTML(song.artist || 'Unbekannter Künstler')}</p>
  ...
  renderEmptyState() {
    ...
    return `
      ...
      <div class="flex flex-col items-center justify-center py-16 text-center" data-role="empty-state">
        ...
        <p class="text-sm text-white/50 mb-6">Füge deinen ersten Song hinzu!</p>
  ...
  showAddSongModal() {
    ...
    App.showFormModal({
      ...
      onSubmit: (formData) => {
        ...
        App.showNotification('Song erfolgreich hinzugefügt!', 'success');
  ...
  startPlaylistTest() {
    ...
    if (!this.filteredSongs || this.filteredSongs.length === 0) {
      ...
      App.showNotification('Keine Songs für die Playlist vorhanden', 'warning');
  ...
  deleteSong(songId) {
    ...
    App.confirm(`Song "${song.title}" wirklich löschen?`, () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        ...
        try {
          ...
          App.showNotification('Import fehlgeschlagen: Ungültige JSON-Datei', 'error');
